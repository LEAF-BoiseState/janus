...
Profits = crpdec.GeneratePrices(Nt)
...
tenure = get_nass.TenureArea('ID', NASS_countyList, NASS_yr, variables)  # tenure from individual counties can also be used
ages = get_nass.Ages(NASS_yr, 'ID')

AgeCDF = get_nass.makeAgeCDF(ages)
TenureCDF = get_nass.makeTenureCDF(tenure)

AgentArray = init_agent.PlaceAgents(Ny, Nx, lc, key_file, 'SRB')

domain = init_agent.InitializeAgents(AgentArray, domain, dist2city, TenureCDF, AgeCDF, switch, Ny, Nx, lc, p)
    ...
    for j in np.arange(Ny):
        ...
        for k in np.arange(Nx):
            ...
            if domain[j,k].FarmerAgents:
                ...
                # Assess Profit
                Profit_last, Profit_pred = crpdec.AssessProfit(CropID_all[i-1, j, k], Profits[i-1,:], Profits[i,:], Nc, CropIDs)

                # Decide on Crop
                CropChoice, ProfitChoice = crpdec.DecideN(domain[j,k].FarmerAgents[0].alpha, domain[j,k].FarmerAgents[0].beta, fmin, fmax, n, Profit_last, CropIDs, Profit_pred, rule=True)
                CropID_all, Profit_ant, Profit_act = crpdec.MakeChoice(CropID_all[i-1,j,k], Profit_last, Profit_pred, CropChoice, ProfitChoice, Profit_act)

                # move these indicies into the input variables
                CropChoice, ProfitChoice = crpdec.DecideN(domain[j,k].FarmerAgents[0].alpha, domain[j,k].FarmerAgents[0].beta, fmin, fmax, n, Profit_last, CropIDs, Profit_pred, rule=True)
                CropID_all[i,j,k], Profit_ant[i,j,k], Profit_act[i,j,k] = crpdec.MakeChoice(CropID_all[i-1,j,k], Profit_last, Profit_ant, CropChoice, ProfitChoice, seed = False) #is there a way to set this up so you can pass a NULL value or no value when seed=False?
