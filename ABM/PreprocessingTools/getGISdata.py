...
counties_shp= gp.read_file('Counties/Counties_SRB_clip_SingleID.shp')
...
def getGISextent(countyList, scale):
    ...
    if scale == '3km':
        SRB_poly= gp.read_file(DataPath+'SRB_poly_3km_V2.shp') 
    elif scale == '1km':
        SRB_poly= gp.read_file(DataPath+'SRB_poly_1km_V2.shp') 
...
np.save(DataPath+'gcam_3km_2010_AdaCanyon.npy', gcam_init)

#this should be updated to only having the functions and the function call goes somewhere else
